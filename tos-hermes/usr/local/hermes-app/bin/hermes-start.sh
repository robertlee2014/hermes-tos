...
LOG_DIR="/var/log/hermes-app"
LOG_FILE="${LOG_DIR}/hermes.log"
PID_FILE="/var/run/hermes-app/hermes.pid"
SOCKET_FILE="/var/api/hermes-app.sock"

APP_DIR="/usr/local/hermes-app"

# Read target volume from preinst
if [ -f "${APP_DIR}/.data_volume" ]; then
    TARGET_VOLUME=$(cat "${APP_DIR}/.data_volume")
else
...
diagnose_env() {
    ...
    log_msg "DATA_DIR=${DATA_DIR} (on ${TARGET_VOLUME})"
    ...
    if [ ! -d "${AGENT_DIR}" ]; then
        log_msg "ERROR: hermes-agent directory not found at ${AGENT_DIR}"
        return 1
    fi
...
start_process() {
    ...
    export HERMES_AGENT_DIR="${AGENT_DIR}"
